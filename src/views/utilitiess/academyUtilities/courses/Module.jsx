...
import { fetchAcademyCourses, fetchAcademyModules } from 'views/API/kpscApi';
...
const UpSkillModules = () => {
  ...
  const fetchModules = async () => {
    try {
      const res = await fetchAcademyModules(headers);
      ...
      if (fetchedData) {
        const tableData = fetchedData.map((p) => ({
          ...
          courseName:
            p.academicCourseDtoList && p.academicCourseDtoList.length > 0 ? p.academicCourseDtoList[0].courseName : 'No Course Name',
  ...
  const fetchCourses = async () => {
    try {
      const res = await fetchAcademyCourses(headers);
      const fetchedData = res.data.content;
      if (fetchedData) {
